...
void ht_remove(ht *table, const char *key)
{
    ...
    while (table->entries[index].key != NULL)
    {
        if (strcmp(key, table->entries[index].key) == 0)
        {
            // Found key, free the associated key and value.
            free((void *)table->entries[index].key);
            free((void *)table->entries[index].value);

            // Mark this slot as deleted by setting the key to NULL.
            table->entries[index].key = NULL;
            table->entries[index].value = NULL;
            table->length--;

            // Rehash any subsequent entries to fill the gap.
            index++;
            while (table->entries[index].key != NULL)
            {
                ...
                table->entries[index].value = NULL;
                table->length--;
                ht_set_entry(table->entries, table->capacity, entry.key, entry.value, &table->length);
                index++;
            }
            return;
        }
